...
##############################################################################################################
 ...
 Script to deploy the Barracuda CloudGen WAF into Microsoft Azure. This script also creates the
...
New-AzResourceGroupDeployment -Name "Deploy_$rg" -ResourceGroupName "$rg" `
                                   -TemplateFile "azuredeploy.json" -TemplateParameterFile "azuredeploy.parameters.json" `
                                   -adminPassword $password -prefix $prefix